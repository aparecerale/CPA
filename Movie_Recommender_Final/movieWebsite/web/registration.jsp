...
<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8"%>
<%@ page import="myPackage.register" %>
<%@ page import="myPackage.loginAuth" %>
                    ...
                    <label>
                        ...
                        <%
                            ...
                            if (!request.getParameter("Name").isEmpty()
                               && !request.getParameter("Password").isEmpty() && !request.getParameter("PasswordConfirm").isEmpty() ) {
                            ...
                            if (register.duplicate(request.getParameter("Name"))  ){
                                ...
                                //int user_id = loginAuth.findUserId(Name);
                        %>
                        <!--
                        JDBC 驱动名及数据库 URL
                        ...
                        <sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
                                           url="jdbc:mysql://localhost:3306/info?useUnicode=true&characterEncoding=utf-8"
                                           user="root"  password="zcpqweqwe123"/>
                        <!--
                        插入数据
                         -->
                        <sql:update dataSource="${snapshot}" var="insertion">
                            INSERT INTO user_info (user_name,password) VALUES ('<%=Name%>', '<%=Password%>');
                        </sql:update>
                        <%
                            int user_id = loginAuth.findUserId(Name);
